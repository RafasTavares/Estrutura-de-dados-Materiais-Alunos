...
public class ArvoreBinariaBuscaNova<T extends Comparable<T>> {
	...
	/*
	 * public boolean Inserir(NoNovo valor) { NoNovo noAux = new NoNovo(valor);
	 * 
	 ...
	 * 
	 * if (this.pai == null) { this.pai = NovoNo;
	 * System.out.println("this.pai: " + this.pai.getElemento()); return true;
	 * 
	...
	private NoNovo<T> remover(NoNovo<T> no, T elemento) {
		if (no == null) {
			...
			if (no.getFilhoEsquerdo() == null) {
				...
				no.filhoEsquerdo = remover(no.getFilhoEsquerdo(),
						no.getElemento());
			}
		} else {
	...
	public T getMaior() {
		NoNovo<T> aux;
		for (aux = this.pai; aux.getFilhoDireito() != null; aux = aux
				.getFilhoDireito()) {
		}
	...
	private T Antecessor(NoNovo<T> no) {
		...
		for (noAux = no; noAux.getFilhoEsquerdo() != null; noAux = noAux
				.getFilhoEsquerdo()) {
		}
		no = noAux;
	...
	/*
	 * public String toString() { listaPreOrdem = retornaLista(); String saida =
	 * "["; for (Object item : listaPreOrdem) { saida += item + ", "; } return
	...
	public NoNovo<T> buscarPrefixado(NoNovo<T> no, T elemento) {
		...
		if (no != null) {
			if (noAux.getElemento().equals(no.getFilhoEsquerdo().getElemento())
					|| noAux.getElemento().equals(
							no.getFilhoDireito().getElemento())) {
				return noAux;
	...
	private int AlturaArvore(NoNovo<T> aux) {
		...
		if (aux.getFilhoEsquerdo() == null && aux.getFilhoDireito() == null) {
			...
			return 1 + Math.max(AlturaArvore(aux.getFilhoEsquerdo()),
					AlturaArvore(aux.getFilhoDireito()));
		}
	}

	private void NumeroBalanceamentoNo(NoNovo<T> no) {
		no.setFB(AlturaArvore(no.getFilhoDireito())
				- AlturaArvore(no.getFilhoEsquerdo()));
	}
	...
	public String ImprimirMaiorPrioridade() {
